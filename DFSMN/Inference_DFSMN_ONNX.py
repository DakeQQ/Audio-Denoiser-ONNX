...
onnx_model_A        = str(parent_path / "DFSMN_Optimized" / "DFSMN.onnx")                                        # The optimized onnx model path.
test_noisy_audio    = model_audio_path("dfsmn")                                                                  # The noisy audio path.
save_denoised_audio = str(parent_path / "speech_with_noise_48k_denoised.wav")                                      # The output denoised audio path.
...
if not isinstance(shape_value_in, int):
    INPUT_AUDIO_LENGTH = max(MAX_DYNAMIC_AUDIO_SECONDS * IN_SAMPLE_RATE, audio_len)  # You can adjust it.
...
if audio_len > INPUT_AUDIO_LENGTH:
    ...
    if fold_active:
        # Batch-fold path: pad with zeros (silence passes through the analysis conv/mask/ISTFT and is trimmed off).
        pad_block = np.zeros((*audio.shape[:-1], pad_amount), dtype=audio.dtype)
    else:
        final_slice = audio[:, :, -pad_amount:].astype(np.float32)
        pad_block = (np.sqrt(np.mean(final_slice * final_slice)) * np.random.normal(loc=0.0, scale=1.0, size=(*audio.shape[:-1], pad_amount))).astype(audio.dtype)
    audio = np.concatenate((audio, pad_block), axis=-1)
elif audio_len < INPUT_AUDIO_LENGTH:
    if fold_active:
        # Batch-fold path: pad with zeros.
        pad_block = np.zeros((*audio.shape[:-1], INPUT_AUDIO_LENGTH - audio_len), dtype=audio.dtype)
    else:
        audio_float = audio.astype(np.float32)
        pad_block = (np.sqrt(np.mean(audio_float * audio_float)) * np.random.normal(loc=0.0, scale=1.0, size=(*audio.shape[:-1], INPUT_AUDIO_LENGTH - audio_len))).astype(audio.dtype)
    audio = np.concatenate((audio, pad_block), axis=-1)
...
audio_duration = out_audio_len / OUT_SAMPLE_RATE if OUT_SAMPLE_RATE > 0 else 0.0
