...
def _vorbis_window(length):
    indices = np.arange(length)
    sine = np.sin(0.5 * np.pi * (indices + 0.5) / (length / 2))
    return torch.from_numpy(np.sin(0.5 * np.pi * sine * sine).astype(np.float32))
