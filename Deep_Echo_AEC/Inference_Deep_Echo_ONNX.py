...
onnx_model_A         = str(parent_path / "Deep_Echo_AEC_Optimized" / "Deep_Echo_AEC.onnx")  # The optimized onnx model path.
test_near_end_audio  = model_audio_path("deep_echo_aec", "near_end")                       # The near end audio path.
test_far_end_audio   = model_audio_path("deep_echo_aec", "far_end")                        # The far end audio path.
...
ORT_Accelerate_Providers  = []  # The mixed FP16 Deep Echo graph is validated on CUDA. Use [] for CPU fallback.
...
far_end_audio_len = far_end_audio.shape[-1]
...
if not isinstance(shape_value_in, int):
    INPUT_AUDIO_LENGTH = max(MAX_DYNAMIC_AUDIO_SECONDS * IN_SAMPLE_RATE, min_len)  # Default to slice in 30 seconds. You can adjust it.
