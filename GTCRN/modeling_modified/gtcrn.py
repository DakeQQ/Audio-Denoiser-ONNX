...
class ERB(nn.Module):
    ...
    def bm(self, x):
        """x: (B,C,T,F)"""
        x_low, x_high = torch.split(x, [self.erb_subband_1, x.shape[-1] - self.erb_subband_1], dim=-1)
        return torch.cat([x_low, self.erb_fc(x_high)], dim=-1)

    def bs(self, x_erb):
        """x: (B,C,T,F_erb)"""
        x_erb_low, x_erb_high = torch.split(x_erb, [self.erb_subband_1, x_erb.shape[-1] - self.erb_subband_1], dim=-1)
        return torch.cat([x_erb_low, self.ierb_fc(x_erb_high)], dim=-1)
...
class SFE(nn.Module):
    ...
    def forward(self, x):
        """x: (B,C,T,F)"""
        return self.unfold(x).reshape(x.shape[0], -1, x.shape[2], x.shape[3])
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.N_layers = len(self.de_convs)
        self.N_layers_minus = self.N_layers - 1

    def forward(self, x, en_outs):
        for i in range(self.N_layers):
            x = self.de_convs[i](x + en_outs[self.N_layers_minus - i])
        return x
...
class Mask(nn.Module):
    ...
    def forward(self, mask, real_part, imag_part):
        m0, m1 = torch.chunk(mask, 2, dim=0)
