...
model_path          = str(parent_path / "H_GTCRN_Optimized" / "H_GTCRN.onnx")  # The optimized onnx model path.
test_noisy_audio    = model_audio_path("h_gtcrn")                             # The noisy audio path.
...
def pad_audio_tail(audio: np.ndarray, target_length: int, fold_active: bool) -> np.ndarray:
    # Batch-fold path: pad with zeros (silence passes through STFT->WPE->AuxIVA->net->ISTFT and is trimmed off).
...
def _build_session_opts_ort() -> onnxruntime.SessionOptions:
    ...
    opts.graph_optimization_level = onnxruntime.GraphOptimizationLevel.ORT_DISABLE_ALL  # ORT 1.26 bug: runtime constant folding creates tensors it cannot handle; torch already did constant folding at export.
...
def main():
    ...
    model_channels = shape_value_in_channel if isinstance(shape_value_in_channel, int) else N_CHANNELS

    # Load the input audio with the channel count expected by the ONNX graph.
    print(f"\nTest Input Audio: {test_noisy_audio}")
    audio = np.array(
        AudioSegment.from_file(test_noisy_audio).set_channels(model_channels).set_frame_rate(IN_SAMPLE_RATE).get_array_of_samples(),
        dtype=np.int16,
    )
    audio = audio.reshape(-1, model_channels)
    audio = normalise_audio(audio, input_dtype_np)
    audio_len = audio.shape[0]

    # Reshape audio to (1, model_channels, audio_len) for model input
    audio = audio.T.reshape(1, model_channels, -1)
    ...
    if audio_len > input_audio_length:
        if (shape_value_in != shape_value_out) and isinstance(shape_value_in, int) and isinstance(shape_value_out, int):
