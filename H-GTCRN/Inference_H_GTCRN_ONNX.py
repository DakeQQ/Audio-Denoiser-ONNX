...
model_path          = r"/home/DakeQQ/Downloads/H_GTCRN_Optimized/H_GTCRN.onnx"      # The optimized onnx model path.
...
N_CHANNELS = 2                          # H-GTCRN expects two microphone channels.
...
def main():
    ...
    audio = np.array(
        AudioSegment.from_file(test_noisy_audio).set_channels(N_CHANNELS).set_frame_rate(IN_SAMPLE_RATE).get_array_of_samples(),
        dtype=np.int16,
    )
