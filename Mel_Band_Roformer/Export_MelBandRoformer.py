...
INPUT_AUDIO_LENGTH = 44100              # Set for static axis export: the length of the audio input signal (in samples) is recommended to be greater than 22050 and less than 220500. Higher values yield better quality but time consume. It is better to set an integer multiple of the HOP_LENGTH value.
...
class MelBandRoformer_Modified(torch.nn.Module):
    ...
    def forward(self, audio):
        ...
        real_L, real_R = torch.chunk(real_part, 2, dim=0)
        imag_L, imag_R = torch.chunk(imag_part, 2, dim=0)
...
if audio_len > INPUT_AUDIO_LENGTH:
    ...
    white_noise = (np.sqrt(np.mean(final_slice * final_slice)) * np.random.normal(loc=0.0, scale=1.0, size=(1, shape_value_in_channel, pad_amount))).astype(audio.dtype)
    audio = np.concatenate((audio, white_noise), axis=-1)
elif audio_len < INPUT_AUDIO_LENGTH:
    white_noise = (np.sqrt(np.mean(audio * audio)) * np.random.normal(loc=0.0, scale=1.0, size=(1, shape_value_in_channel, INPUT_AUDIO_LENGTH - audio_len))).astype(audio.dtype)
    audio = np.concatenate((audio, white_noise), axis=-1)
