...
if audio_len > INPUT_AUDIO_LENGTH:
    ...
    if fold_active:
        pad_block = np.zeros((1, shape_value_in_channel, pad_amount), dtype=audio.dtype)
    else:
        final_slice = audio[:, :, -pad_amount:].astype(np.float32)
        pad_block = (np.sqrt(np.mean(final_slice * final_slice, dtype=np.float32), dtype=np.float32) * np.random.normal(loc=0.0, scale=1.0, size=(1, shape_value_in_channel, pad_amount))).astype(audio.dtype)
    audio = np.concatenate((audio, pad_block), axis=-1)
elif audio_len < INPUT_AUDIO_LENGTH:
    if fold_active:
        pad_block = np.zeros((1, shape_value_in_channel, INPUT_AUDIO_LENGTH - audio_len), dtype=audio.dtype)
    else:
        audio_float = audio.astype(np.float32)
        pad_block = (np.sqrt(np.mean(audio_float * audio_float, dtype=np.float32), dtype=np.float32) * np.random.normal(loc=0.0, scale=1.0, size=(1, shape_value_in_channel, INPUT_AUDIO_LENGTH - audio_len))).astype(audio.dtype)
    audio = np.concatenate((audio, pad_block), axis=-1)
...
sf.write(save_denoised_audio, denoised_wav, OUT_SAMPLE_RATE, format='WAVEX', subtype='PCM_16' if output_dtype_np == np.int16 else 'FLOAT')
