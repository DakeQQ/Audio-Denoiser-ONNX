...
class MelBandRoformer(Module):
    ...
    def _get_mono_freq_indices(self, device='cpu'):
        if (self._mono_freq_indices is None) or (self._mono_freq_indices.device != device):
            num_bands = self.freqs_per_band.shape[0]
            repeated = torch.arange(self.num_freqs, device=device).expand(num_bands, -1)
            self._mono_freq_indices = repeated[self.freqs_per_band]
        return self._mono_freq_indices

    def forward(self, stft_repr, zeros, target=None, return_loss_breakdown=False):
