...
Static-graph, ONNXRuntime-friendly export. The reusable module performs its
optimization in PyTorch __init__() and forward().
...
Optimization summary vs original:
  ...
  - No batch-dependent branching in forward.
    - Static ISTFT precomputes the exact COLA denominator and removes one runtime
        ConvTranspose; dynamic ISTFT retains runtime normalization.
    - Static output slice bounds avoid dynamic shape/index arithmetic.
...
class STFT_Process(torch.nn.Module):
    ...
    def __init__(
        ...
        self._static_istft = bool(static_frames)
        ...
        if model_type in ('istft_A', 'istft_B'):
            self._build_istft_kernels(n_fft, f_bins, window, hop_len, max_frames, self._static_istft)
    ...
    def _build_istft_kernels(self, n_fft, f_bins, window, hop_len, n_frames, static_frames):
        ...
        if static_frames:
            # COLA normalization depends only on the configured static frame count.
            # Baking it removes a full ConvTranspose and its temporary output.
...
def main():
    ...
    with torch.no_grad():
        ...
        print(f"  opset:        {OPSET}")
        print(f"  post-export:  onnxslim (standalone STFT utility only)")
