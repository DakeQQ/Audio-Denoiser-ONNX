...
if len(out_meta_A) != 2:
    raise ValueError(f"Expected two ONNX source outputs, got {len(out_meta_A)}.")
in_name_A = in_meta_A.name
out_name_A0 = out_meta_A[0].name
out_name_A1 = out_meta_A[1].name
input_dtype_np = numpy_dtype_from_onnx_meta(in_meta_A)
output_dtype_np_0 = numpy_dtype_from_onnx_meta(out_meta_A[0])
output_dtype_np_1 = numpy_dtype_from_onnx_meta(out_meta_A[1])


# Load the input audio
print(f"\nTest Input Audio: {test_mixed_audio}")
audio_segment = AudioSegment.from_file(test_mixed_audio).set_frame_rate(IN_SAMPLE_RATE)
input_shape = in_meta_A.shape
...
if input_dtype_np == np.float16:
    dtype_limit = np.finfo(input_dtype_np).max
    audio = np.nan_to_num(audio, nan=0.0, posinf=dtype_limit, neginf=-dtype_limit)
audio = audio.reshape(-1, input_channels).T[np.newaxis, ...]
audio = np.concatenate(
    (np.zeros((*audio.shape[:-1], PAD_HEAD), dtype=audio.dtype), audio),
    axis=-1,
)
...
def process_segment(_inv_audio_len, _slice_start, _slice_end, _audio):
    ...
    return (
        _slice_start * _inv_audio_len,
        np.array(outputs[0].numpy(), copy=True),
...
if output_dtype_np_0 == np.float16:
    separated_0 = separated_0.astype(np.float32)
if output_dtype_np_1 == np.float16:
    separated_1 = separated_1.astype(np.float32)
sf.write(save_separated_0, separated_0, OUT_SAMPLE_RATE, subtype='PCM_16' if output_dtype_np_0 == np.int16 else 'FLOAT')
sf.write(save_separated_1, separated_1, OUT_SAMPLE_RATE, subtype='PCM_16' if output_dtype_np_1 == np.int16 else 'FLOAT')
