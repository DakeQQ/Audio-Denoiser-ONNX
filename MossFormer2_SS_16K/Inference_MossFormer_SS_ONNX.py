...
in_meta_A = ort_session_A.get_inputs()[0]
out_meta_A = ort_session_A.get_outputs()
in_name_A = in_meta_A.name
out_name_A0 = out_meta_A[0].name
out_name_A1 = out_meta_A[1].name
input_dtype_np = _numpy_dtype_from_meta(in_meta_A)
output_dtype_np = _numpy_dtype_from_meta(out_meta_A[0])


# Load the input audio
print(f"\nTest Input Audio: {test_mixed_audio}")
audio = np.array(AudioSegment.from_file(test_mixed_audio).set_channels(1).set_frame_rate(IN_SAMPLE_RATE).get_array_of_samples(), dtype=np.int16)
audio = normalise_audio(audio, input_dtype_np)
if input_dtype_np == np.float16:
    audio = np.nan_to_num(audio, nan=0.0, posinf=np.finfo(np.float16).max, neginf=-np.finfo(np.float16).max)
audio = np.concatenate([np.zeros(PAD_HEAD, dtype=audio.dtype), audio], axis=0)
audio_len = len(audio)
...
pad_head_out = int(round(PAD_HEAD * INPUT_TO_OUTPUT_SCALE))
...
output_buffer_0 = _ort_zeros((1, 1, output_audio_length), output_dtype_np)
output_buffer_1 = _ort_zeros((1, 1, output_audio_length), output_dtype_np)
binding_A = ort_session_A.io_binding()
binding_A.bind_ortvalue_input(in_name_A, input_buffer)
binding_A.bind_ortvalue_output(out_name_A0, output_buffer_0)
binding_A.bind_ortvalue_output(out_name_A1, output_buffer_1)
...
def process_segment(_inv_audio_len, _slice_start, _slice_end, _audio):
    ...
    return _slice_start * _inv_audio_len, np.array(output_buffer_0.numpy(), copy=True), np.array(output_buffer_1.numpy(), copy=True)


# Start to run MossFormer_SS
print("\nRunning the MossFormer_SS by ONNX Runtime.")
results = []
...
separated_0 = np.concatenate(saved_0, axis=-1).reshape(-1)[pad_head_out:out_audio_len]
separated_1 = np.concatenate(saved_1, axis=-1).reshape(-1)[pad_head_out:out_audio_len]
end_time = time.time()
...
if output_dtype_np == np.float16:
    separated_0 = separated_0.astype(np.float32)
    separated_1 = separated_1.astype(np.float32)
sf.write(save_separated_0, separated_0, OUT_SAMPLE_RATE, subtype='PCM_16' if output_dtype_np == np.int16 else 'FLOAT')
sf.write(save_separated_1, separated_1, OUT_SAMPLE_RATE, subtype='PCM_16' if output_dtype_np == np.int16 else 'FLOAT')
elapsed = end_time - start_time
audio_duration = max(out_audio_len - pad_head_out, 0) / OUT_SAMPLE_RATE if OUT_SAMPLE_RATE > 0 else 0.0
