...
MAX_SIGNAL_LENGTH = 2048 if DYNAMIC_AXES else (INPUT_AUDIO_LENGTH // 50 + 1)  # Max frames for audio length after STFT processed. Set a appropriate larger value for long audio input, such as 4096.
...
class MOSSFORMER_SE(torch.nn.Module):
    def __init__(self, mossformer_se, stft_model, istft_model, sample_rate):
    ...
    def forward(self, audio):
        ...
        audio = self.istft_model(final_real * factor, final_imag * factor)
        if SAMPLE_RATE_SCALE < 1.0:
            audio *= norm_factor
...
with torch.inference_mode():
    ...
    mossformer = MOSSFORMER_SE(mossformer, custom_stft, custom_istft, SAMPLE_RATE)
...
if isinstance(shape_value_in, str):
    INPUT_AUDIO_LENGTH = max(6 * SAMPLE_RATE, audio_len)
else:
