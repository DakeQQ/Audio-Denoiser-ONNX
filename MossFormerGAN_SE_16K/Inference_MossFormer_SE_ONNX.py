    ...
    provider_options = [
        {
            ...
            'cudnn_conv_algo_search': 'EXHAUSTIVE',      # ["DEFAULT", "HEURISTIC", "EXHAUSTIVE"]   The MossFormer_SE error out, try using DEFAULT.
            'sdpa_kernel': '2',                           # ["0", "1", "2"]
            'use_tf32': '1',
...
if not isinstance(shape_value_in, int):
    INPUT_AUDIO_LENGTH = min(MAX_DYNAMIC_AUDIO_SECONDS * IN_SAMPLE_RATE, audio_len)
...
if audio_len > INPUT_AUDIO_LENGTH:
    ...
    zero_pad = np.zeros((*audio.shape[:-1], pad_amount), dtype=audio.dtype)
    audio = np.concatenate((audio, zero_pad), axis=-1)
elif audio_len < INPUT_AUDIO_LENGTH:
    zero_pad = np.zeros((*audio.shape[:-1], INPUT_AUDIO_LENGTH - audio_len), dtype=audio.dtype)
    audio = np.concatenate((audio, zero_pad), axis=-1)
...
audio_seconds = float(out_audio_len) / float(OUT_SAMPLE_RATE) if OUT_SAMPLE_RATE > 0 else 0.0
rtf = elapsed_seconds / audio_seconds if audio_seconds > 0.0 else float('inf')
