...
DYNAMIC_AXES = False         # Opt in only when validating dynamic standalone transforms.
...
class STFT_Process(torch.nn.Module):
    ...
    def __init__(
        ...
        self._istft_center_pad = center_pad
    ...
    def _stft_A_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._center_pad:
            if self._pad_mode == 'reflect':
                ...
                x = torch.cat([self.padding_zero, x, self.padding_zero], dim=2)
...
def main():
    ...
    with torch.no_grad():
        ...
        graph_mode = "dynamic_axes=True" if DYNAMIC_AXES else "static_graph=True"
        print(f"Export  STFT={STFT_TYPE}, ISTFT={ISTFT_TYPE}, opset={OPSET}, {graph_mode}\n")
        ...
        dynamic_axes_summary = "audio_len (dim 2), n_frames (dim 2)" if DYNAMIC_AXES else "None (static I/O)"
        print(f"  dynamic_axes: {dynamic_axes_summary}")
        print(f"  opset:        {OPSET}")
        print("  post-export:  None (no onnxslim/simplifier)")
        print(f"  remaining_dynamic_behavior: {'I/O axes' if DYNAMIC_AXES else 'None'}")
