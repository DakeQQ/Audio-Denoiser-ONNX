...
class ZipEnhancer(nn.Module):
    ...
    def forward(self, noisy_mag, noisy_pha):  # [B, F, T]
        x = self.TSConformer(self.dense_encoder(torch.cat((noisy_mag.unsqueeze(-1).permute(0, 3, 2, 1), noisy_pha.unsqueeze(-1).permute(0, 3, 2, 1)), dim=1)))
        return self.mask_decoder(x).squeeze(1).transpose(1, 2), self.phase_decoder(x).squeeze(1).transpose(1, 2)
